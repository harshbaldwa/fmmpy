...
def test_calc_inner_p2(backend):
    ...
    part_z = np.array([0.12, 0.12, 0.12, 0.12], dtype=np.float32)
    ...
    res_loc = loc_exp(in_val, in_x, in_y, in_z, index_r[0]*num_p2, part_x, 
                     part_y, part_z, 0, num_p2)
    ...
    assert abs(res_loc - res_direct)/res_direct < 2e-3
...
def test_trans_inner_p2(backend):
    ...
    assert abs(res_loc - res_direct)/res_direct < 5e-3